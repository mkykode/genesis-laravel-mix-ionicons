...
add_filter( 'body_class', 'local695_body_class_post' );
/**
 ...
 */
function local695_body_class_post( $classes ) {
...
add_action( 'genesis_before', 'local695_setup_full_width' );
/**
 * Runs function if post has featured image and full-width content layout.
 *
 * @since 1.0.0
 */
function local695_setup_full_width() {
	...
	add_action( 'genesis_after_header', 'local695_entry_background_post' );
	/**
	 * Outputs background image.
	 *
	 * @since 1.0.0
	 */
	function local695_entry_background_post() {
	...
	add_action( 'genesis_entry_header', 'local695_gravatar_post', 7 );
	/**
	 * Outputs Gravatar.
	 *
	 * @since 1.0.0
	 */
	function local695_gravatar_post() {
