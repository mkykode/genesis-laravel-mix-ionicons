...
add_filter( 'woocommerce_style_smallscreen_breakpoint', 'local695_woocommerce_breakpoint' );
/**
 * Modifies the WooCommerce breakpoints.
 *
 * @since 1.1.0
 */
function local695_woocommerce_breakpoint() {
...
add_filter( 'genesiswooc_default_products_per_page', 'local695_default_products_per_page' );
/**
 ...
 */
function local695_default_products_per_page() {
	return 8;
}

add_filter( 'woocommerce_pagination_args', 'local695_woocommerce_pagination' );
/**
 ...
 */
function local695_woocommerce_pagination( $args ) {

	$args['prev_text'] = sprintf( '&laquo; %s', __( 'Previous Page', 'local695' ) );
	$args['next_text'] = sprintf( '%s &raquo;', __( 'Next Page', 'local695' ) );

	return $args;

}

add_action( 'after_switch_theme', 'local695_woocommerce_image_dimensions_after_theme_setup', 1 );
/**
 * Defines WooCommerce image sizes on theme activation.
 *
 * @since 1.1.0
 */
function local695_woocommerce_image_dimensions_after_theme_setup() {
	...
	local695_update_woocommerce_image_dimensions();

}

add_action( 'activated_plugin', 'local695_woocommerce_image_dimensions_after_woo_activation', 10, 2 );
/**
 ...
 */
function local695_woocommerce_image_dimensions_after_woo_activation( $plugin ) {
 ...
 * @since 1.1.0
 */
function local695_update_woocommerce_image_dimensions() {
...
add_filter( 'woocommerce_get_image_size_gallery_thumbnail', 'local695_gallery_image_thumbnail' );
/**
 ...
 */
function local695_gallery_image_thumbnail( $size ) {
